...
namespace SCTVTelevision
{
	...
	public class TaskbarNotifier : System.Windows.Forms.Form
	{
		...
		public bool slideUp = false;//if true the notification bar slides up from the bottom otherwise it slides in from the right
		#endregion
			...
			switch (taskbarState)
			{
				case TaskbarStates.hidden:
					...
					if(slideUp)
						...
						SetBounds(WorkAreaRectangle.Right-1, WorkAreaRectangle.Bottom-BackgroundBitmap.Height, 0, BackgroundBitmap.Height);
		...
		public void Show(playSchedule theShow)
		{
			TVProgramme prog = (TVProgramme)theShow.theProgramme;
			Show(prog.Title +" on "+ prog.Channel +" at "+ prog.StartTime.ToShortTimeString(), prog.Description, 500, 10000, 500);
		}

		public void Show(ShowNotification theShow)
		{
			Show(theShow.name +" on "+ theShow.channel.ToString() +" at "+ theShow.StartTime.ToShortDateString(), theShow.description, 500, 10000, 500);
		}

		public void Show(TVProgramme prog)
		{
			Show(prog.Title +" on "+ prog.Channel +" at "+ prog.StartTime.ToShortTimeString(), prog.Description, 500, 10000, 500);
		}
		...
		protected void OnTimer(Object obj, EventArgs ea)
		{
			...
			{
				case TaskbarStates.appearing:
					if(slideUp)//slide the bar up from the bottom
					{
					...
					else//slide the bar in from the right
					{
						if (Width < BackgroundBitmap.Width)
							SetBounds(Left-nIncrementShow, Top,Width + nIncrementShow, Height);
						else
						{
							timer.Stop();
							Height = BackgroundBitmap.Height;
				...
				case TaskbarStates.disappearing:
					...
					{
						if(slideUp)//slide the bar down to the bottom
						{
						...
						else//slide the bar to the right
						{
							if (Left < WorkAreaRectangle.Right)
								SetBounds(Left + nIncrementHide, Top, Width - nIncrementHide, Height);
							else
								Hide();
						}
					}
		...
		private void InitializeComponent()
		{
			...
			this.Font = new System.Drawing.Font("Comic Sans MS", 14.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
