...
namespace SCTVTelevision
{
	...
	public class ColumnStyle:DataGridTextBoxColumn
	{
		...
		Color bgColor;
		public ColumnStyle(Color backColor)
		{
			//
...
//				BackBrush = Brushes.Blue;
			Brush newBrush = new SolidBrush(bgColor);
//			BackBrush = backColor;
//			else
//				BackBrush = Brushes.White;

			g.FillRectangle(newBrush, Bounds.X, Bounds.Y, Bounds.Width, Bounds.Height);

			System.Drawing.Font font = new Font(System.Drawing.FontFamily.GenericSansSerif , (float)8.25 );
			g.DrawString( GetColumnValueAtRow(Source, RowNum).ToString() ,font ,Brushes.Black ,Bounds.X ,Bounds.Y );
		}
