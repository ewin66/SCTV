...
namespace SCTV
{
    ...
    public class TabCtlEx : System.Windows.Forms.TabControl
    {
        ...
        private bool canCloseAllTabs = false;
        ...
        public bool CanCloseAllTabs
        {
            get
            {
                return this.canCloseAllTabs;
            }
            set
            {
                this.canCloseAllTabs = value;
            }
        }
                ...
                for (int nIndex = 0; nIndex < this.TabCount; nIndex++)
                {
                    if (nIndex != this.SelectedIndex)
                    {
                        ...
                        using (LinearGradientBrush _Brush = new LinearGradientBrush(tabTextArea, SystemColors.Control, SystemColors.ControlLight, LinearGradientMode.Vertical))
                        {
                            ...
                            if (this.TabCount > 1 || canCloseAllTabs)
                            {
        ...
        {
            if (!canCloseAllTabs && ((TabPageEx)this.TabPages[nIndex]).Menu != null)
                return true;
            return false;
        }
        ...
        protected override void OnMouseLeave(EventArgs e)
        {
            ...
            for (int nIndex = 0; nIndex < this.TabCount; nIndex++)
            {
                if (nIndex != this.SelectedIndex && canCloseAllTabs)
                {
                    tabTextArea = (RectangleF)this.GetTabRect(nIndex);
                    GraphicsPath _Path = new GraphicsPath();
                    _Path.AddRectangle(tabTextArea);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (!DesignMode)
            {
                ...
                for (int nIndex = 0; nIndex < this.TabCount; nIndex++)
                {
                    ...
                    if (tabTextArea.Contains(pt) && canCloseAllTabs)
                    {
                        using (
        ...
        {
            if (!DesignMode && SelectedIndex > 0 || canCloseAllTabs)
            {
