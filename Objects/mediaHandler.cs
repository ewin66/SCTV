...
namespace SCTVObjects
{
	...
	public class MediaHandler
	{
        ...
        static string mediaFilePath = Application.StartupPath +"\\config\\media.xml";
        static string gameFilePath = Application.StartupPath +"\\config\\games.xml";
        static string mediaFileDefaultPath = "config/media_default.xml";
        string playlistPath = Application.StartupPath + "\\config\\playlist.xml";
        string playlistDefaultPath = Application.StartupPath + "\\config\\playlist_default.xml";
        string imagesPath = Application.StartupPath + "\\config\\images.xml";
        string imagesDefaultPath = Application.StartupPath + "\\config\\images_default.xml";
        string locationFilePath = Application.StartupPath + "\\config\\locations.xml";
        string fileTypeFilePath = Application.StartupPath + "\\config\\fileTypes.xml";
        static string mediaTypesPath = Application.StartupPath + "\\config\\onlineMedia.xml";
        string DefaultPathToSaveTo = Application.StartupPath + "\\DVD\\";
		...
		public MediaHandler()
		{
            ...
            if (File.Exists(fileTypeFilePath))
            ...
            if (File.Exists(locationFilePath))
                xmlLocations.Load(locationFilePath);
            else
            {
                //add base node to tempDoc
        ...
        /// </summary>
        /// <returns></returns>
        public static DataSet GetMedia()
        {
            try
            {
                dsMedia = new DataSet();
                ...
                try
                {
                    if (File.Exists(mediaFilePath))
                        dsMedia.ReadXml(new StringReader(xmlQuery(mediaFilePath, "getMedia.xqu").InnerXml.ToString()));
                }
                catch (Exception ex)
                {
                ...
                {
                    if (File.Exists(mediaFileDefaultPath))
                        dsMedia.ReadXml(new StringReader(xmlQuery(mediaFileDefaultPath, "getMedia.xqu").InnerXml.ToString()));

                    //DataTable dtEmptyTable = new DataTable();
                    //dsMedia.Tables.Add(dtEmptyTable);
                }
                else if (dsMedia.Tables[0] != null)
                {
                    try
                    {
        ...
        /// </summary>
        /// <returns></returns>
        public static DataSet GetMediaTypes()
        {
            try
            {
        ...
        public Media GetSequel(Media currentFile, bool continousPlay)
        {
            ...
            if (fileNameNums.Length > 0 && int.TryParse(fileNameNums, out intFileNameNums))
            {
                ...
                dsMedia.Tables[0].DefaultView.RowFilter = "filePath LIKE '*" + fileNameBase + "*'";
            ...
            if (sequel == null)
            {
                ...
                dsMedia.Tables[0].DefaultView.RowFilter = "title LIKE '*" + titleBase + "*'";
        ...
        private static DataSet validateMedia(DataSet dsMedia)
        {
            try
            {
                ...
                //save xml
                dsMedia.WriteXml(mediaFilePath);
            }
            catch (Exception ex)
            {
        ...
        private void findLocations()
        {
            foreach (XmlNode location in xmlLocations["locations"])
                if (location.InnerText.ToString().Trim().Length > 0 && !locations.Contains(location.ToString().ToLower()))
                    locations.Add(location.InnerText.ToString().Trim());

            //add DefaultPathToSaveTo to locations 
            //TODO: get this value from config file
            if (!locations.Contains(Application.StartupPath.ToLower() + "\\" + DefaultPathToSaveTo.ToLower()))
                locations.Add(Application.StartupPath.ToLower() + "\\" + DefaultPathToSaveTo.ToLower());
        }
...
//			return dsMedia;
                ...
                //{
                    ...
                    foreach (string cat in categories)
                    {
                        if (category.Length > 0)
                            category += " and category LIKE '*" + cat + "*'";
                        else
                    ...
                    switch (mediaType.ToLower())
                    {
                        case "pictures":
                            switch (category.ToLower().Replace("category like '*","").Replace("*'",""))
                            {
                                ...
                                case "misc":
                                    if (startsWith.ToLower() != "all")
                                        dsMedia.Tables[0].DefaultView.RowFilter = "len(category) = 0 and SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*'";
                                    else
                                        dsMedia.Tables[0].DefaultView.RowFilter = "len(category) = 0 and MediaType LIKE '*" + mediaType + "*'";
                                ...
                                case "new":
                                    ...
                                    if (startsWith.ToLower() != "all")
                                        sortableDt.DefaultView.RowFilter = "SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*' and timesPlayed = '0'";
                                    else
                                        sortableDt.DefaultView.RowFilter = "MediaType LIKE '*" + mediaType + "*' and timesPlayed = '0'";
                                ...
                                case "not-so popular":
                                    ...
                                    if (startsWith.ToLower() != "all")
                                        sortableDt.DefaultView.RowFilter = "SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*'";
                                    else
                                        sortableDt.DefaultView.RowFilter = "MediaType LIKE '*" + mediaType + "*'";
                                ...
                                default:
                                    if (startsWith.ToLower() != "all")
                                        dsMedia.Tables[0].DefaultView.RowFilter = category + " and SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*'";
                                        //dsMedia.Tables[0].DefaultView.RowFilter = "category LIKE '*" + category + "*' and SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*'";
                                    else
                                        dsMedia.Tables[0].DefaultView.RowFilter = category + " and MediaType LIKE '*" + mediaType + "*'";
                        ...
                        case "online":
                            ...
                            if (startsWith.ToLower() == "all")
                                dsMediaTypes.Tables[0].DefaultView.RowFilter = category + " and mediaCategory LIKE '*" + mediaType + "*'";
                                //dsMediaTypes.Tables[0].DefaultView.RowFilter = "category LIKE '*" + category + "*' and mediaCategory LIKE '*" + mediaType + "*'";
                            else
                                dsMediaTypes.Tables[0].DefaultView.RowFilter = category + " and mediaCategory LIKE '*" + mediaType + "*' and name LIKE '" + startsWith + "*'";
                        ...
                        default: //movies and tv
                            ...
                            switch (category.ToLower().Replace("category like '*", "").Replace("*'", ""))
                            {
                                ...
                                case "recent":
                                    ...
                                    if (startsWith.ToLower() != "all")
                                        sortableDt.DefaultView.RowFilter = "SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*' and len(LastPlayed) > 0";
                                    else
                                        sortableDt.DefaultView.RowFilter = "MediaType LIKE '*" + mediaType + "*' and len(LastPlayed) > 0";
                                ...
                                case "star rating":
                                    ...
                                    if (startsWith.ToLower() != "all")
                                        dsMedia.Tables[0].DefaultView.RowFilter = "SortBy LIKE '" + startsWith + "*' and MediaType LIKE '*" + mediaType + "*' and len(stars) > 0";
                                    else
                                        dsMedia.Tables[0].DefaultView.RowFilter = "MediaType LIKE '*" + mediaType + "*' and len(stars) > 0";
                                ...
                                case "justin downloads":
                                    ...
                                    sortableDt.DefaultView.RowFilter = "Category LIKE '*" + category + "*'";
                                ...
                                foreach (DataRowView dr in foundData)
                                {
                                    try
                                    {
                                        if (dr["filePath"].ToString().ToLower().Contains("justindownloads"))
                                        {
                                            ...
                                            if (tempDT.Rows.Count == 0 || tempDT.Select("filePath like '%" + currentTitle + "_%'").Length == 0)
                                            {
        ...
        /// <returns></returns>
		public static DataView GetAllCategories(string mediaType)
		{
			try
			{
                if (mediaType.ToLower() == "online")
                {
                    ...
                    {
                        dsMediaTypes.Tables[0].DefaultView.RowFilter = "len(category) > 0 and mediaCategory LIKE '*" + mediaType + "*'";
        ...
        /// </summary>
        /// <returns></returns>
        public static ArrayList GetAllMediaTypes()
        {
...
//			Tools.WriteToFile(Tools.errorFile,"Results: "+ expr.Execute(col).ToXml().ToString());
        ...
        private XmlDocument lookForMedia(bool bestMatch, string location, XmlDocument tempDoc, DataSet ds)
        {
                ...
                foreach (FileSystemInfo entry in fl.GetFileSystemInfos())
                {
                    if (entry.Extension.Length > 0)//this is a file
                    {
                        ...
                        if (compareExtension(entry.Extension))
                        {
                            ...
                            else if (ds.Tables[0].DefaultView.Table.Select("(len(IMDBNum) = 0 or len(category) = 0) and filePath = '" + dLocation + "\\" + entry.Name.ToString().Trim() + "'").Length > 0)
                            {
                                try
                                {
                                    //try to get imdb info for this one
                                    IMDBScraper imdb = new IMDBScraper();
                                    Media newMedia = imdb.getInfoByTitle(entry.Name.ToString().Trim(), bestMatch);

                                    ds.Tables[0].DefaultView.RowFilter = "(len(IMDBNum) = 0 or len(category) = 0) and filePath = '" + dLocation + "\\" + entry.Name.ToString().Trim() + "'";
            ...
            {
                Tools.WriteToFile(Tools.errorFile, "lookForMedia2 " + e.Message);
            ...
            if (!isDuplicate(videoDoc, fileToAdd.FullName, "mediaFiles/media/filePath"))
            {
                try
                {
                    if (foundMediaType.ToLower() == "movies" || foundMediaType.ToLower() == "tv")
                    {
                        ...
                        Media newMedia = imdb.getInfoByTitle(formattedFileName, bestMatch);
                ...
                if (imdb != null)
                {
                    ...
                    goofsNode.InnerText = imdb.Goofs;
                    triviaNode.InnerText = imdb.Trivia;
                    shortDescriptionNode.InnerText = imdb.ShortDescription;
                }
                else
                {
        ...
        public static string FormatNameString(string stringToSpace)
        {
            try
            {
                if (stringToSpace != null && stringToSpace.Length > 0)
                {
                    ...
                    if (stringToSpace.LastIndexOf(".") == stringToSpace.Length - 4)
        ...
        public void AddLocations(ArrayList newLocations, bool removeMissingLocations)
        {
            ...
            {
                locationCorrectCase = System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(location);//correct the casing

                if (!locations.Contains(locationCorrectCase) && location.ToLower() != Application.StartupPath.ToLower() + "\\" + DefaultPathToSaveTo.ToLower())//keep out duplicates and the DefaultPathToSaveTo
                {
            ...
            if (removeMissingLocations)//need to remove any locations that aren't in newLocations
            {
                foreach (string location in locations)
                {
                    ...
                    if (!newLocations.Contains(location))//find the location not in newLocations
                    {
                        updatedLocations = true;

                        locations.Remove(locationCorrectCase);
                    }
                }
            }

            if (updatedLocations) //there were new locations added
                xmlLocations.Save(locationFilePath);
        }
        ...
        public static void UpdateMediaInfo(Media updatedMedia)
        {
            try
            {
                //if (dsMedia.Tables.Count < 1)//check for the existance of tables in dsMedia
                ...
                if (updatedMedia.filePath.ToLower().Contains("justindownloads"))
                {
                    ...
                    foreach (DataRowView dr in sortableDt.DefaultView)
                    {
                        ...
                        dsMedia.Tables[0].DefaultView.RowFilter = "filePath='" + relatedMedia.filePath + "'";
                ...
                {
                    //find the given media
                    dsMedia.Tables[0].DefaultView.RowFilter = "filePath='" + updatedMedia.filePath + "'";
                    DataView dv = dsMedia.Tables[0].DefaultView;
            ...
            try
            {
                ...
                try
                {
                    if (File.Exists(playlistPath))
                        dsPlaylist.ReadXml(new StringReader(xmlQuery(playlistPath, "getPlaylist.xqu").InnerXml.ToString()));
                ...
                if (dsPlaylist.Tables.Count < 1)
                {
                    if (File.Exists(playlistDefaultPath))
                        dsPlaylist.ReadXml(new StringReader(xmlQuery(playlistDefaultPath, "getPlaylist.xqu").InnerXml.ToString()));
                    else
        ...
        public DataSet GetImages()
        {
            try
            {
                ...
                {
                    if (File.Exists(imagesPath))
                        dsImages.ReadXml(new StringReader(xmlQuery(imagesPath, "getImages.xqu").InnerXml.ToString()));
                ...
                {
                    if (File.Exists(imagesPath))
                        dsImages.ReadXml(new StringReader(xmlQuery(imagesDefaultPath, "getImages.xqu").InnerXml.ToString()));
        ...
        /// </summary>
        public static void AddOnlineMedia(string name, string url, string mediaCategory, string category, string description, string coverImage)
        {
